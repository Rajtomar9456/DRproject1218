...
class Customers extends Model
{
    ...
    public function getter(){
        ...
        $user = User::sortable(['id'=>'ASC'])
            ...
            'address_book.entry_state as entry_state', 'countries.countries_id', 'countries.countries_name', 'zones.zone_id', 'zones.zone_name')
            ->where('role_id',2)
            ->groupby('users.id')
            ->get();
        ...
        switch ( $filter )
        {
            case 'Name':
                $user = DB::table('users')
                    ...
                    'address_book.entry_street_address as entry_street_address', 'address_book.entry_suburb as entry_suburb',
                    'address_book.entry_postcode as entry_postcode', 'address_book.entry_city as entry_city',
                    'address_book.entry_state as entry_state', 'countries.countries_id', 'countries.countries_name', 'zones.zone_id', 'zones.zone_name','image_categories.path as path')
