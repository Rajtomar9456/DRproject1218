...
class Reports extends Model
{
    ...
    public function customersReportTotal($request)
    {
        ...
        foreach($orders as $order){
            $data = DB::select('SELECT * FROM `orders_status_history` WHERE orders_id = '.$order->orders_id.' ORDER BY `orders_status_history`.`orders_status_history_id` DESC LIMIT 1');
            foreach($data as $d){
    ...
    public function outofstock($request)
    {
        $report = DB::table('inventory')
                ...
                $notInsertedinInentory = DB::select('SELECT products.products_id,products_description.products_name FROM `products` LEFT JOIN products_description ON products_description.products_id = products.products_id WHERE products_description.language_id = 1 AND products.products_id NOT IN (SELECT products_id FROM inventory GROUP BY products_id)');
